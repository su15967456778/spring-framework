...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public AbstractApplicationContext() {
		//加载资源模式解析器 用来加载我们系统运行的时候需要的资源（xml）
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			2.设置活跃状态为true
			3.设置关闭状态为false
			4.获取Environment对象，并加载当前系统的属性到Environment中
			5.准备监听器和事件的集合对象，默认为空的对象集合
			 */
			prepareRefresh();
	...
	protected void initMessageSource() {
		//获取bean工厂，一般是DefailtListableBeanFactory
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		//判断是否已有xml文件定义了id为messageSource的bean对象
		if (beanFactory.containsLocalBean(MESSAGE_SOURCE_BEAN_NAME)) {
			//如果有，则取这个对象
			this.messageSource = beanFactory.getBean(MESSAGE_SOURCE_BEAN_NAME, MessageSource.class);
			//当父类bean工厂不为空，并且这个对象是 HierarchicalMessageSource 类型
			// Make MessageSource aware of parent MessageSource.
			if (this.parent != null && this.messageSource instanceof HierarchicalMessageSource) {
				//类型强制转换，转换为HierarchicalMessageSource的类型
				HierarchicalMessageSource hms = (HierarchicalMessageSource) this.messageSource;
				//判断父类MessageSource是否为空，如果等于空，则设置父类的messageSource
	...
	protected void registerListeners() {
		...
		// 从容器中获取所有实现了ApplicaitonListener接口的bd的bdName
		// 放入Multicaster中
		...
		//此处先早发布的监听器集合 第一步执行过这个类 prepareRefresh
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		//为上下文准备初始化类型转换器 比如numberToStringService
		...
		//如果beanFactory之前没有注入嵌入值解析器，则注册默认的解析器，主要用于注解属性的解析
		if (!beanFactory.hasEmbeddedValueResolver()) {
			beanFactory.addEmbeddedValueResolver(strVal -> getEnvironment().resolvePlaceholders(strVal));
		}

		// Initialize LoadTimeWeaverAware beans early to allow for registering their transformers early.
		//初始化weaverAwareNames，以便注册他们的转换器
